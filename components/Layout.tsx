...
export const Layout: React.FC<LayoutProps> = ({ children, onDashboardClick, showDashboardLink, isAdmin }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-zinc-950 text-gray-900 dark:text-gray-100 transition-colors duration-300">
      {/* Sticky Header */}
      <motion.nav 
        className={`fixed top-0 left-0 right-0 z-[60] transition-all duration-300 backdrop-blur-md ${
            isScrolled || isAdmin
            ? 'bg-white/90 dark:bg-zinc-900/90 shadow-sm py-3 border-b border-gray-200 dark:border-zinc-800' 
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col">
                    <span className={`text-xl font-bold tracking-tight transition-opacity opacity-100 hidden sm:block leading-none`}>
                        LuxeMenu
                    </span>
                    {isAdmin && (
                        <span className="text-[10px] uppercase font-bold text-gold-600 dark:text-gold-500 tracking-wider">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
                {showDashboardLink && !isAdmin && (
                    <button 
                        ...
                        className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-700 transition-colors text-xs font-bold mr-2 border border-gray-200 dark:border-zinc-700"
                        title={t.providerDashboard[language]}
                    >
                        <LayoutDashboard className="w-4 h-4" />
                        <span className="hidden sm:inline">{t.providerDashboard[language]}</span>
                    </button>
                )}
                <button 
