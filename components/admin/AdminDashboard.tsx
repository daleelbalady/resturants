...
export const AdminDashboard = () => {
  ...
  const stats = {
    ...
    activeOrders: MOCK_ORDERS.filter(o => o.status !== 'delivered').length,
  ...
  const handleSaveProduct = (product: MenuItem) => {
    if (editingItem) {
        setMenuItems(prev => prev.map(p => p.id === product.id ? product : p));
        setEditingItem(null);
    } else {
                            ...
                            {MOCK_ORDERS.slice(0,3).map(order => (
                                <div key={order.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-zinc-800/50 rounded-xl">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="font-bold text-gray-900 dark:text-white">{t.orderId[language]}{order.id.split('-')[1]}</p>
                        ...
                        >
                            <div className="flex-1">
                                <div className="flex items-center gap-3 mb-2">
                                    <h3 className="text-xl font-bold text-gray-900 dark:text-white">#{order.id.split('-')[1]}</h3>
                                    <OrderBadge status={order.status} />
                            ...
                            <div className="flex flex-col justify-between items-end min-w-[200px]">
                                ...
                                <div className="flex gap-2 mt-4">
                                    <button className="px-4 py-2 bg-gray-100 dark:bg-zinc-800 rounded-lg font-bold text-xs hover:bg-gray-200 dark:hover:bg-zinc-700">
                                        {t.cancelOrder[language]}
                                    </button>
                                    <button className="px-4 py-2 bg-gold-500 text-white rounded-lg font-bold text-xs hover:bg-gold-600">
